...
import com.silence.myblog.util.MarkDownUtil;
import com.silence.myblog.util.PageQueryUtil;
import com.silence.myblog.util.PageResult;
import com.silence.myblog.util.PatternUtil;
...
public class BlogServiceImpl implements BlogService {
    ...
    public String saveBlog(Blog blog) {
        ...
        if (blogMapper.insertSelective(blog) > 0) {
            ...
            //FIXME 为什么批量插入的，插入一个就算成功？难道不是全都插入成功才算成功吗？
    ...
    public PageResult getBlogsForIndexPage(int page) {
        ...
        List<Blog> blogList = blogMapper.findBlogList(pageUtil);
        //返回给前端的blog和后台的blog不是同一个类，因为有很多信息是只面向后台的，出于安全性不能对前端开放
        //故需要单独用BlogListVO类返回
        List<BlogListVO> blogListVOS = getBlogListVOsByBlogs(blogList);
        int total = blogMapper.getTotalBlogs(pageUtil);
        //将要返回的数据用pageResult包装
